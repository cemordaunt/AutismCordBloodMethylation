...
DMRbackPercent <- mapply(function(x, y){c("Hyper  " = sum(x$width[x$percentDifference > 0]) * 100 / sum(y$width), 
                                       "Hypo" = sum(x$width[x$percentDifference < 0]) * 100 / sum(y$width))},
                      x = append(DiscDMRs, RepDMRs), y = append(DiscDMRbackground, RepDMRbackground))
...
DMRbackground_auto <- lapply(append(DiscDMRbackground, RepDMRbackground), subset, chr %in% paste("chr", 1:22, sep = ""))
...
DMRbackground_chrX <- lapply(append(DiscDMRbackground, RepDMRbackground), subset, chr == "chrX")
