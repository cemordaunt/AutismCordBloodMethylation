...
# Data ####
# DMRs
DMRs <- loadRegions("DMRs/Discovery/Diagnosis 50/DMR_smoothed_methylation_Dx_Discovery50.txt",
...
raw <- loadRegions("DMRs/Discovery/Diagnosis 50/DMR_raw_methylation_Dx_Discovery50.txt", 
                   chroms = c(paste("chr", 1:22, sep = ""), "chrM"), sort = TRUE)
raw$DMRid <- paste("DMR", 1:nrow(raw), sep = "_")
raw <- cbind(raw[,c("chr", "start", "end", "DMRid", colnames(raw)[grepl("JLCM", colnames(raw), fixed = TRUE)])])

# Candidates
candidates <- loadRegions("DMRs/Discovery/Diagnosis 50/CandidateRegions_Dx_Discovery50.csv",
                          chroms = c(paste("chr", 1:22, sep = ""), "chrM"), sort = TRUE)
...
# Manhattan plot
CMplotDMR(candidates, prefix = "Figures/Diagnosis DMRs", plot.type = "m")

# QQ plot
CMplotDMR(candidates, prefix = "Figures/Diagnosis DMRs", plot.type = "q")
rm(candidates)
# Heatmap ####
# Meth Data
...
# Plot Heatmap
pdf(file="Figures/Diagnosis DMRs Methylation Heatmap.pdf", width = 10, height = 8, onefile = FALSE)
...
ggbiplotPCA(data.pca = data.pca, groups = phenoData$Diagnosis, pc = c(1,2), 
            file = "Figures/ Diagnosis DMRs PCA by Diagnosis.png", xlim = c(-7, 7), ylim = c(-7, 7))
ggbiplotPCA(data.pca = data.pca, groups = phenoData$Study, pc = c(1,2), 
            file = "Figures/ Diagnosis DMRs PCA by Study.png", xlim = c(-7, 7), ylim = c(-7, 7),
...
ggbiplotPCA(data.pca = data.pca, groups = phenoData$Sex, pc = c(1,2), 
            file = "Figures/ Diagnosis DMRs PCA by Sex.png", xlim = c(-7, 7), ylim = c(-7, 7),
...
covStats <- DMRmethLm(DMRs = meth$DMRid, catVars = catVars, contVars = contVars, sampleData = samples_cov, 
                      file = "Tables/Diagnosis DMRs Methylation by Covariate Stats.txt")

covSum <- DMRmethLmSum(covStats, file = "Tables/Diagnosis DMRs Methylation by Covariate Summary.txt")
...
covHeatmap(covStats, variableOrdering = "manual", regionOrdering = "variable", variables = variables,
           ...
           file = "Figures/Diagnosis DMRs Covariate Heatmap Sorted by Diagnosis.png")
covHeatmap(covStats, variableOrdering = "hierarchical", regionOrdering = "hierarchical", 
           file = "Figures/Diagnosis DMRs Covariate Heatmap Clustered.png")
...
DMRs_anno <- getDMRanno(DMRstats = DMRs, regDomains = regDomains, file = "Tables/Diagnosis DMRs Annotation.txt")
...
prepGREAT(DMRs = makeGRange(DMRs, direction = "all"), Background = makeGRange(background, direction = "all"), 
          fileName = "UCSC Tracks/Diagnosis DMRs Discovery hg19 for GREAT.bed", writeBack = TRUE,
          backName = "UCSC Tracks/Diagnosis Background Discovery hg19 for GREAT.bed", 
...
prepGREAT(DMRs = makeGRange(DMRs, direction = "hyper"), Background = makeGRange(background, direction = "all"), 
          fileName = "UCSC Tracks/Diagnosis Hyper DMRs Discovery hg19 for GREAT.bed", writeBack = FALSE,
          chroms = c(paste("chr", 1:22, sep = ""), "chrM"))
prepGREAT(DMRs = makeGRange(DMRs, direction = "hypo"), Background = makeGRange(background, direction = "all"), 
          fileName = "UCSC Tracks/Diagnosis Hypo DMRs Discovery hg19 for GREAT.bed", writeBack = FALSE,
          chroms = c(paste("chr", 1:22, sep = ""), "chrM"))

# Get Enrichments from GREAT
BEDfile_Back <- "UCSC Tracks/Diagnosis Background Discovery hg19 for GREAT.bed.gz"
greatAll <- getGREATenrichments(BEDfile_DMR = "UCSC Tracks/Diagnosis DMRs Discovery hg19 for GREAT.bed.gz", 
                                BEDfile_Back = BEDfile_Back)
greatHyper <- getGREATenrichments(BEDfile_DMR = "UCSC Tracks/Diagnosis Hyper DMRs Discovery hg19 for GREAT.bed.gz", 
                                  BEDfile_Back = BEDfile_Back)
greatHypo <- getGREATenrichments(BEDfile_DMR = "UCSC Tracks/Diagnosis Hypo DMRs Discovery hg19 for GREAT.bed.gz", 
                                 BEDfile_Back = BEDfile_Back)
...
write.table(greatCombined, "Tables/Diagnosis DMRs Discovery GREAT Combined Results.txt", sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)
# No enriched terms
...
# Plot Heatmap
pdf(file="Figures/Diagnosis DMRs Raw Methylation Heatmap.pdf", width = 10, height = 8, onefile = FALSE)
buildHeatmap(x = methdiff, phenoData = phenoData[,c("Sample", "Diagnosis", "Study")], hm.low = - hm.lim, hm.high = hm.lim, 
             pheno.breaks = c("TD", "ASD", "MARBLES", "EARLI"), 
             pheno.values = c("TD" = "#3366CC", "ASD" = "#FF3366", "MARBLES" = "#FFFF33", "EARLI" = "#FF6633")) %>%
        printHeatmap
...
ggbiplotPCA(data.pca = data.pca, groups = phenoData$Diagnosis, pc = c(1,2), 
            file = "Figures/Diagnosis DMRs Raw Meth PCA by Diagnosis.png", xlim = c(-7, 7), ylim = c(-7, 7))
ggbiplotPCA(data.pca = data.pca, groups = phenoData$Study, pc = c(1,2), 
            file = "Figures/Diagnosis DMRs Raw Meth PCA by Study.png", xlim = c(-7, 7), ylim = c(-7, 7),
            breaks = c("MARBLES", "EARLI"), values = c("MARBLES" = "#3366CC", "EARLI" = "#FF3366"),
            legend.position = c(0.77, 1.03))
ggbiplotPCA(data.pca = data.pca, groups = phenoData$Sex, pc = c(1,2), 
            file = "Figures/Diagnosis DMRs Raw Meth PCA by Sex.png", xlim = c(-7, 7), ylim = c(-7, 7),
            breaks = c("M", "F"), values = c("M" = "#3366CC", "F" = "#FF3366"),
            legend.position = c(0.77, 1.03))
rm(data.pca)
...
# Get Stats
covStats <- DMRmethLm(DMRs = meth$DMRid, catVars = catVars, contVars = contVars, sampleData = samples_cov, 
                      file = "Tables/Diagnosis DMRs Raw Methylation by Covariate Stats.txt")

covSum <- DMRmethLmSum(covStats, file = "Tables/Diagnosis DMRs Raw Methylation by Covariate Summary.txt")
...
covHeatmap(covStats, variableOrdering = "manual", regionOrdering = "variable", variables = variables,
           sortVariable = "Diagnosis_Alg", 
           file = "Figures/Diagnosis DMRs Raw Meth Covariate Heatmap Sorted by Diagnosis.png")
covHeatmap(covStats, variableOrdering = "hierarchical", regionOrdering = "hierarchical", 
           file = "Figures/Diagnosis DMRs Raw Meth Covariate Heatmap Clustered.png")
...
# Missing Data
# Error with DMR_8 and PE
# Error with DMR_26 and DM1or2
# Error with DMR_97 and DM1or2
# Error with DMR_216 and PE
...
# Plot All Results
greatCombined$name <- factor(greatCombined$name, levels = rev(unique(greatCombined$name)))
plotGREAT(greatCombined, file = "Figures/Males Diagnosis Discovery GREAT Plot.png", axis.text.y.size = 11, 
          axis.text.y.width = 35, legend.position = c(1.2, 0.8), width = 6, height = 3.5)
...
data.pca <- prcomp(t(meth_heat), center = TRUE, scale. = TRUE)
ggbiplotPCA(data.pca = data.pca, groups = phenoData$Diagnosis, pc = c(1,2), 
            file = "Figures/Males Diagnosis DMRs Raw Meth PCA by Diagnosis.png", xlim = c(-35, 35), ylim = c(-35, 35))
ggbiplotPCA(data.pca = data.pca, groups = phenoData$Study, pc = c(1,2), 
            file = "Figures/Males Diagnosis DMRs Raw Meth PCA by Study.png", xlim = c(-35, 35), ylim = c(-35, 35),
            breaks = c("MARBLES", "EARLI"), values = c("MARBLES" = "#3366CC", "EARLI" = "#FF3366"),
            legend.position = c(0.77, 1.03))
rm(data.pca)
...
# Error with DMR_22 and DM1or2
# Error with DMR_56 and SmokeYN_Pregnancy
# Error with DMR_101 and DM1or2
# Error with DMR_171 and DM1or2
# Error with DMR_266 and DM1or2
# Error with DMR_412 and DM1or2
...
plotGREAT(greatPlotData, file = "Figures/Females Diagnosis Discovery GREAT Plot GO Terms.png", axis.text.y.size = 10, 
          axis.text.y.width = 35, legend.position = c(1.29, 0.87))
...
greatPlotData <- subset(greatPlotData, ID %in% # Get top 10 from each direction
                                unique(c(greatPlotData$ID[greatPlotData$Direction == "All"][1:10], 
                                         ...
                                         greatPlotData$ID[greatPlotData$Direction == "Hypo"][1:10])))
greatPlotData$name <- factor(greatPlotData$name, levels = rev(unique(greatPlotData$name)))
plotGREAT(greatPlotData, file = "Figures/Females Diagnosis Discovery GREAT Plot Pathways.png", axis.text.y.size = 10, 
          axis.text.y.width = 35, legend.position = c(1.29, 0.87))
...
greatPlotData <- subset(greatPlotData, ID %in% # Get top 10 from each direction
                                unique(c(greatPlotData$ID[greatPlotData$Direction == "All"][1:10], 
                                         ...
                                         greatPlotData$ID[greatPlotData$Direction == "Hypo"][1:10])))
greatPlotData$name <- factor(greatPlotData$name, levels = rev(unique(greatPlotData$name)))
plotGREAT(greatPlotData, file = "Figures/Females Diagnosis Discovery GREAT Plot Immune.png", axis.text.y.size = 9, 
          axis.text.y.width = 35, legend.position = c(1.28, 0.87))

rm(greatAll, greatCombined, greatHyper, greatHypo, background, DMRs, BEDfile_Back, samples)

# Filter DMRs by covariate associations, remove DMRs associated with technical vars or not associated with Dx
#       Then try heatmap and PCA again
# Rerun comparisons and exclude chrX chrY before getting DMRs (Running on epigenerate)
